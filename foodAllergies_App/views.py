...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if User.objects.filter(username=username).exists():
            ...
            return Response(
                {"Response": "Email already exists"}, status=status.HTTP_400_BAD_REQUEST
            )
            ...
            if serializer.is_valid():
                ...
                nuser = users.objects.create(
                    user=user, profile_pic=profile_pic, phone=phone, is_doctor=is_doctor
                )
                ...
                return Response(
                    {"token": token, "userdata": data}, status=status.HTTP_201_CREATED
                )
            else:
...
class FoodAllergyViewSet(viewsets.ModelViewSet):
    def get_queryset(self):
        return (
            FoodAllergy.objects.all()
            .select_related("food")
    ...
    search_fields = [
        "food__arabicName",
        "food__englishName",
        "category__arabicName",
        "category__englishName",
        "allergy__arabicDescription",
        "allergy__englishDescription",
        "allergy__arabicName",
        "allergy__englishName",
        "allergy__arabicSymptoms",
        "allergy__englishSymptoms",
        "allergy__arabicPrevention",
        "allergy__englishPrevention",
    ]
    filterset_fields = [
        "food__arabicName",
        "food__englishName",
...
class VerifyEmail_viewSet(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, pk=None):
        ...
        if email_verification.verified:
            return Response(
                {"detail": "Email already verified"}, status=status.HTTP_302_FOUND
            )
